...
export default function LoginPage() {
  ...
  const handleClick = async () => {
    ...
    const { success, token, currentUserId, currentUserAccount, message } =
      await login({
        account,
        password,
      });
    ...
    if (success) {
      ...
      Swal.fire({
        ...
        title: `
            <div class="${styles["my-custom-title"]}">
              <div class="${styles["my-custom-title-text"]}">${
          message === "Not a valid account" ? "帳號不存在" : message
        }！</div>
              <div class="${styles["my-custom-title-icon"]}">
                <img src="${redIcon}" alt="cross" class="${
          styles["my-custom-image"]
        }" />
              </div>
            </div>
          `,
      ...
      <div className={`${styles.loginDiv} mx-auto`}>
        <div className={`${styles.loginContainer}`}>
          ...
          <div className={styles.aContainer}>
            <Link to="/regist">
              <span className={styles.aLink}>註冊</span>
            </Link>
            <div>・</div>
            <Link to="/admin">
              <span className={styles.aLink}>後台登入</span>
            </Link>
          </div>
        </div>
      </div>
